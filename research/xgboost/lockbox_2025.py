"""
Lockbox 2025 -- true-holdout validation of the margin model.

2025 data was never pullable during model development (the pipeline 404'd on
it until the 2026-07-12 nflverse_compat fix), so no modeling decision was ever
influenced by 2025 outcomes. This is the exact lockbox_2024.py protocol
shifted one season forward:

  1. Calibrate Student-t on OOS residuals (train 2018-2023, resid on 2024).
  2. Train final XGBRegressor on 2018-2024 with target = margin.
  3. Predict every 2025 team-week, P(cover) at the market spread.
  4. Per-game argmax pick, pushes excluded, score against actual covers.

No hyperparameter changes. No reruns. One shot; the number is the number.
Note: dakota_rolling_shifted is NaN for all 2025 rows (dropped upstream) --
identical to live serving conditions going forward.

Output:
  research/xgboost/lockbox_2025_picks.csv
  research/xgboost/lockbox_2025_summary.json
"""
...
FEATURES = ROOT / "features_2025.parquet"
PICKS_OUT = ROOT / "lockbox_2025_picks.csv"
SUMMARY_OUT = ROOT / "lockbox_2025_summary.json"
...
def load_margin_target() -> pd.DataFrame:
    ...
    sched = nfl.import_schedules(list(range(2018, 2026)))
...
def main():
    ...
    # ----- 1. Calibration: train on 2018-2023, residuals on 2024
    print("\n[1/3] OOS calibration (train 2018-2023, residuals on 2024)...")
    tr_cal = df[df["season"] < 2024]
    cv = df[df["season"] == 2024]
    ...
    # ----- 2. Final training: 2018-2024
    print("\n[2/3] Final training on 2018-2024...")
    tr_final = df[df["season"] < 2025]
    m_final = xgb.XGBRegressor(**XGB_REG_KW)
    m_final.fit(tr_final[feat], tr_final["margin"])

    # ----- 3. Predict on 2025
    print("\n[3/3] Predicting on 2025 (frozen)...")
    ...
    summary = {
        "model_version": "margin_v2_lockbox_2025",
        "trained_on": "2018-2024",
        "test_on": "2025",
        "features_used": len(feat),
        "dropped": DROP_FEATS,
        "notes": "2025 never available during model development (pipeline 404); "
                 "dakota NaN for all 2025 rows, matching live serving conditions.",
        "calibration": {"nu_t": round(nu, 3), "scale_t": round(scale, 3),
                        "sigma_normal": round(sigma_norm, 3),
                        "resid_n": int(len(resid)),
                        "resid_source": "2024 OOS"},
        "aggregate": {
